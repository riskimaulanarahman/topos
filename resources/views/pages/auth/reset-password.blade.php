...
@section('title', 'Reset Password – TOGA POS')
...
@push('style')
    <style>
        ...
        .form-control{height:48px;border:1px solid var(--border);border-radius:10px;flex:1}
...
<div class="toga-login">
  <div class="toga-left">
    <form method="POST" action="{{ route('password.update') }}" class="js-disable-on-submit" novalidate>
      @csrf
      <input type="hidden" name="token" value="{{ $request->route('token') }}">

      <div class="toga-logo"><img src="{{ asset('img/toga-gold-ts.png') }}" alt="TOGA POS logo"></div>
      <h3 class="toga-title">Reset Password</h3>
      <p class="toga-subtitle">Set a new password for your account.</p>

      <div class="form-group">
        <label for="email">Email</label>
        <div class="input-row">
          <span class="icon-pill"><i class="fa-solid fa-envelope"></i></span>
          <input id="email" type="email" class="form-control @error('email') is-invalid @enderror" name="email" value="{{ old('email', request('email')) }}" autocomplete="email" placeholder="you@company.com" aria-invalid="{{ $errors->has('email') ? 'true' : 'false' }}">
        </div>
      ...
      <div class="form-group">
        ...
        <div class="input-row">
          ...
          <input id="password" type="password" class="form-control @error('password') is-invalid @enderror" name="password" autocomplete="new-password" placeholder="••••••••" aria-invalid="{{ $errors->has('password') ? 'true' : 'false' }}">
          <button type="button" class="toggle-password" data-target="#password" aria-label="Show password" aria-pressed="false"><i class="fa-regular fa-eye"></i></button>
        </div>
      ...
      <div class="form-group">
        ...
        <div class="input-row">
          <span class="icon-pill"><i class="fa-solid fa-lock"></i></span>
          <input id="password_confirmation" type="password" class="form-control" name="password_confirmation" autocomplete="new-password" placeholder="••••••••">
          <button type="button" class="toggle-password" data-target="#password_confirmation" aria-label="Show password" aria-pressed="false"><i class="fa-regular fa-eye"></i></button>
        </div>
      </div>

      <div class="actions"><button type="submit" class="btn btn-gold">Reset Password</button></div>
  ...
  <div class="toga-right" aria-hidden="true">
    <svg viewBox="0 0 600 480" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="POS cashier illustration">
      <defs><linearGradient id="goldGradientRP" x1="0" y1="0" x2="1" y2="1"><stop offset="0%" stop-color="#FFE680"/><stop offset="100%" stop-color="#FFD700"/></linearGradient></defs>
      <rect x="40" y="300" width="520" height="120" rx="16" fill="#F3F4F6"/>
      <rect x="150" y="240" width="160" height="60" rx="10" fill="#E5E7EB"/>
      <rect x="170" y="210" width="180" height="40" rx="8" fill="url(#goldGradientRP)"/>
